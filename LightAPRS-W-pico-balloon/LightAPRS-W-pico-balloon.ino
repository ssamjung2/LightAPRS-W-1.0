...
int8_t  CallNumber = 16; //SSID http://www.aprs.org/aprs11/SSIDs.txt
char    Symbol = '/O'; // '/O' for balloon, '/>' for car, for more info : http://www.aprs.org/symbols/symbols-new.txt
bool    alternateSymbolTable = false ; //false = '/' , true = '\'

char    comment[50] = "Boys and Girls Club Bloomington IL"; // Max 50 char
char    comment2[50] = "Atlas-10"; // Max 50 char

char    StatusMessage[50] = "Atlas-10";
//*****************************************************************************

uint16_t  BeaconWait = 20; //50 seconds sleep for next beacon (HF or VHF). This is optimized value, do not change this if possible.
...
unsigned int   StatusWait=10;   //min mins between status messages
...
//#define WSPR_DEFAULT_FREQ       7040100UL //40m band
...
//enum mode cur_mode = MODE_FT8; // FT8
...
char hf_message[13] = "KW9D AA00";//for non WSPR modes, you don't have to change this, updated by hf_call and GPS location
  ...
  if (readBatt() > BattMin) {
    ...
    if ((gps.location.age() < 1000 || gps.location.isUpdated()) && gps.location.isValid()) {
      if (gps.satellites.isValid() && gps.satellites.value() >= 3) {
        ...
        ublox_high_alt_mode_enabled = false; //gps sleep mode resets high altitude mode.
#if defined(DEVMODE)
        Serial.println(F("Before HF Mode Check"));
#endif
        ...
        if (readBatt() > WsprBattMin && timeStatus() == timeSet && ((minute() % 10 == 3) || (minute() % 10 == 7)) ) {
          ...
          if(gps.time.minute() % StatusWait == 0){
            ...
            blankline(status_buff, 60);          
            sleepSeconds(15);
            sprintf(status_buff, "%s", comment2);
            sendStatus();
            blankline(status_buff, 60);      
          }
...
void updateTelemetry() {
  ...
  sprintf(telemetry_buff + 54, "%s", StatusMessage);
